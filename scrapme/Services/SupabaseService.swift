...
actor SupabaseService {
    ...
    /// Upload direct des événements vers Supabase (alternative à la fonction Edge)
    func uploadEvents(_ events: [EconomicEvent]) async throws {
        guard let url = URL(string: "\(supabaseURL)/rest/v1/economic_calendar") else {
            throw SupabaseError.invalidURL
        }
        
        // Convertir les événements en JSON via Codable
